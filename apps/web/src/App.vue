...
import { Activity, Archive, ArrowRight, Bookmark, Building2, CalendarDays, Check, ChevronRight, CircleHelp, Clock3, Database, ExternalLink, Flame, Footprints, Home, Info, Medal, MessageCircle, Search, Send, Share2, Sparkles, Target, TrendingUp, Trophy, UserRound } from 'lucide-vue-next'
    ...
    <div class="app-main">
      ...
      <main v-if="view==='today'" class="content">
        ...
        <template v-else>
          ...
          <section v-if="detailTab==='overview'" class="tab-panel overview-grid"><article class="about-card"><div class="section-title"><strong>关于 {{ player.zh }}</strong><span>球员档案</span></div><p>{{ player.intro }}</p><div class="stat-row"><div><small>国家</small><strong>{{ player.flag }} {{ player.nation }}</strong></div><div><small>位置</small><strong>{{ player.position }}</strong></div><div><small>职业年代</small><strong>{{ player.years }}</strong></div><template v-if="detail"><div><small>出生日期</small><strong>{{ detail.birthDate }}</strong></div><div><small>身高</small><strong>{{ detail.height }}</strong></div><div><small>惯用脚</small><strong>{{ detail.foot }}</strong></div></template></div></article><div class="overview-side"><article><Medal/><div><small>代表荣誉</small><strong>{{ player.honors[0] }}</strong></div></article><article><Target/><div><small>推荐比赛</small><strong>{{ player.match }}</strong></div></article><article><Sparkles/><div><small>冷知识</small><strong>{{ player.fact }}</strong></div></article></div></section>
